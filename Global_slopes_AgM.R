...
for (j in 1:length(crop)){
  ...
  ArNew[[j]]<-ArNew[[j]][[c(20:50)]]
  ArNewi[[j]]<-brick(ArNewi[[j]])*(ArNew[[j]]/ArNew[[j]])
  ArNewi[[j]]<-ArNewi[[j]][[c(20:50)]]
...
Global_data_A<-list()
setwd("C:/Users/hac809/Desktop/newcrops_sensitivity_old/Chapter2/AgMERRA_sim/")
...
GyieldA<-data.frame("Year"=c(1980:2010),"WYield"=WGlobY, "MYield"=MGlobY, "RYield"=RGlobY)
Global_data_A[[a]]<-GyieldA

GyieldA<-subset(GyieldA,Year>1981)

X11(width=6,height=10)
par(mfrow=c(3,1),mai=c(0.3,0.7,0.2,0.2))
plot(GyieldA$Year,GyieldA$WYield,ylim=c(1,3.5),ylab="Yield (T/ha)",main="Wheat",
     xaxt='n',cex.lab=1.3,font.lab=2)
lines(GyieldA$Year,GyieldA$WYield,col="Blue")
lines(GyieldA$Year,predict(lm(GyieldA$WYield~GyieldA$Year)),col='Red')

plot(GyieldA$Year,GyieldA$MYield,ylim=c(0,6),ylab="Yield (T/ha)",main="Maize",
     xaxt='n',cex.lab=1.3,font.lab=2)
lines(GyieldA$Year,GyieldA$MYield,col="Blue")
lines(GyieldA$Year,predict(lm(GyieldA$MYield~GyieldA$Year)),col='Red')

plot(GyieldA$Year,GyieldA$RYield,ylim=c(1.5,6),ylab="Yield (T/ha)",main="Rice",
     cex.lab=1.3,font.lab=2)
lines(GyieldA$Year,GyieldA$RYield,col="Blue")
lines(GyieldA$Year,predict(lm(GyieldA$RYield~GyieldA$Year)),col='Red')

dev.copy2pdf(file = paste0("Plots/",k,".pdf"))
}

names(Global_data_A)<-unlist(strsplit(YIFiles,split=".",fixed=TRUE))[c(TRUE,FALSE)]
